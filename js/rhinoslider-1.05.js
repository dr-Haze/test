		...
		var
			...
			init = function ($slider, settings, vars) {
				...
				if (settings.controlsPrevNext) {
					...
					buttons = '<a class="' + vars.prefix + 'prev icon-left-open-big ' + vars.prefix + 'btn">' + settings.prevText + '</a><a class="' + vars.prefix + 'next  icon-right-open-big ' + vars.prefix + 'btn">' + settings.nextText + '</a>';
				...
				$(vars.items[0]).find('img').show();

				$slider.css({
					top: 'auto',
					left: 'auto',
					position: 'relative',
					height: $(vars.items[0]).css('height')
					// height: $(vars.items[0]).find('img').css('height')
				});
				...
				if (settings.showBullets !== 'never') {
					...
					var navi = '<ul class="' + vars.prefix + 'bullets fb-home-slider-controls center">';
					vars.items.each(function (i) {
						var $item = $(this);
						var id = vars.prefix + 'item' + i;
						navi = navi + '<li id="' + id + '-bullet" class="' + vars.prefix + 'bullet"></li>';
						// navi = navi + '<li><a id="' + id + '-bullet" class="' + vars.prefix + 'bullet"></a></li>';
					});
					navi = navi + '</ul>';
